...
public class CompartidoSerializer {
    public static class Referencia extends Thread implements cat.ogasoft.protocolizer.SerializerProtoWorker<cat.ogasoft.examples2.Compartido.Referencia> {
        ...
        public void work(cat.ogasoft.examples2.Compartido.Referencia container) {
            ...
            super.setName("cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.ReferenciaWorker");
            start();
        }
    ...
    public static class Identificacion {
        ...
        public static cat.ogasoft.protocolizer.messages.Compartido.Identificacion buildIdentificacion(cat.ogasoft.examples2.Compartido.Identificacion target){
            ...
            if(target.getReferencia() != null){
                builder.setReferencia(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Referencia.buildReferencia(target.getReferencia()));
            }
            if(target.getMarca() != null){
                builder.setMarca(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Marca.buildMarca(target.getMarca()));
            }
            if(target.getOtras() != null){
                if(target.getOtras() != null){
                    for(cat.ogasoft.examples2.Compartido.Referencia k:target.getOtras()){
                        builder.addOtras(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Referencia.buildReferencia(k));
                    }
                }
            }
    ...
    public static class Oferta {
        ...
        public static cat.ogasoft.protocolizer.messages.Compartido.Oferta buildOferta(cat.ogasoft.examples2.Compartido.Oferta target){
            ...
            if(target.getArticuloRegalo() != null){
                builder.setArticuloRegalo(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Articulo.buildArticulo(target.getArticuloRegalo()));
            }
    ...
    public static class Kit {
        ...
        public static cat.ogasoft.protocolizer.messages.Compartido.Kit buildKit(cat.ogasoft.examples2.Compartido.Kit target){
            ...
            if(target.getArticuloKit() != null){
                builder.setArticuloKit(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Articulo.buildArticulo(target.getArticuloKit()));
            }
            if(target.getComponenteKit() != null){
                if(target.getComponenteKit() != null){
                    for(cat.ogasoft.examples2.Compartido.Articulo k:target.getComponenteKit()){
                        builder.addComponenteKit(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Articulo.buildArticulo(k));
                    }
                }
            }
            if(target.getCantidadComponente() != null){
    ...
    public static class Articulo {
        ...
        public static cat.ogasoft.protocolizer.messages.Compartido.Articulo buildArticulo(cat.ogasoft.examples2.Compartido.Articulo target){
            ...
            if(target.getIdentificacion() != null){
                builder.setIdentificacion(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Identificacion.buildIdentificacion(target.getIdentificacion()));
            }
            if(target.getStock() != null){
                builder.setStock(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Stock.buildStock(target.getStock()));
            }
            if(target.getStocks() != null){
                if(target.getStocks() != null){
                    for(cat.ogasoft.examples2.Compartido.Stock k:target.getStocks()){
                        builder.addStocks(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Stock.buildStock(k));
                    }
                }
            }
            if(target.getPrecios() != null){
                if(target.getPrecios() != null){
                    for(cat.ogasoft.examples2.Compartido.Precio k:target.getPrecios()){
                        builder.addPrecios(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Precio.buildPrecio(k));
                    }
                }
            }
            if(target.getReferenciaPrincipal() != null){
                builder.setReferenciaPrincipal(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Referencia.buildReferencia(target.getReferenciaPrincipal()));
            }
            if(target.getReferencias() != null){
                if(target.getReferencias() != null){
                    for(cat.ogasoft.examples2.Compartido.Referencia k:target.getReferencias()){
                        builder.addReferencias(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Referencia.buildReferencia(k));
                    }
                }
            }
            if(target.getDescripcion() != null){
            ...
            if(target.getEquivalentes() != null){
                if(target.getEquivalentes() != null){
                    for(cat.ogasoft.examples2.Compartido.Articulo k:target.getEquivalentes()){
                        builder.addEquivalentes(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Articulo.buildArticulo(k));
                    }
                }
            }
            if(target.getCascoEnvase() != null){
                builder.setCascoEnvase(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Articulo.buildArticulo(target.getCascoEnvase()));
            }
            if(target.getRestricciones() != null){
                builder.setRestricciones(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Restricciones.buildRestricciones(target.getRestricciones()));
            }
            if(target.getTasas() != null){
                if(target.getTasas() != null){
                    for(cat.ogasoft.examples2.Compartido.Tasa k:target.getTasas()){
                        builder.addTasas(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Tasa.buildTasa(k));
                    }
                }
            }
            if(target.getKits() != null){
                if(target.getKits() != null){
                    for(cat.ogasoft.examples2.Compartido.Kit k:target.getKits()){
                        builder.addKits(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Kit.buildKit(k));
                    }
                }
            }
            if(target.getOfertas() != null){
                if(target.getOfertas() != null){
                    for(cat.ogasoft.examples2.Compartido.Oferta k:target.getOfertas()){
                        builder.addOfertas(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Oferta.buildOferta(k));
                    }
                }
            }
    ...
    public static class DatosTaller {
        ...
        public static cat.ogasoft.protocolizer.messages.Compartido.DatosTaller buildDatosTaller(cat.ogasoft.examples2.Compartido.DatosTaller target){
            ...
            builder.setDireccion(cat.ogasoft.protocolizer.dumppers.serializers.CompartidoSerializer.Direccion.buildDireccion(target.getDireccion()));
